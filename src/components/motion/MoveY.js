...
const MoveY = ({ character, comp_id }) => {
  ...
  return (
    <div className="relative h-12">
      <svg ref={svgRef} className="h-16 cursor-pointer" onClick={() => handleClick()}>
        <g>
          <path
      ...
      <div
        ...
        id={comp_id}
        //bg-blue-700
        className={`text-center absolute pointer-events-none top-0 left-0 z-10 rounded w-36 flex justify-center items-center  text-white p-2 my-2 text-xs cursor-pointer `}

      >
        Move Y{" "}
        <input
          type="number"
          className="text-black pointer-events-auto text-center w-16 mx-2"
          inputmode="numeric"
          value={steps}
          ref={inputRef}
