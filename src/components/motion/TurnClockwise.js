...
const TurnClockWise = ({ character, characterAngle, comp_id }) => {
  ...
  return (
    ...
    <div className="relative h-12 " >
      ...
      <div
        id={comp_id}
        className={`flex absolute top-0 left-0 z-10 pointer-events-none rounded  text-white w-52 px-2 py-1 my-1 text-sm cursor-pointer text-center`}

      >
        <div className="flex mx-auto">
          Turn
          <RedoIcon className="mx-2" />  <input
            className="mx-2 p-1 py-0 text-center pointer-events-auto text-black w-8"
            type="number"
            value={angle}
