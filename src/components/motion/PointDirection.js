...
const PointDirection = ({ character, characterAngle, comp_id }) => {
  const [angle, setAngle] = useState(90);

  // handle anti-clockwise rotation
  const handleClick = () => {
    let anti_angle = angle;
    const el = document.getElementById(character.active);
    const character_angle = character.characters.find(
      (x) => x.id === character.active
    );
    if (character_angle) {
      el.style.transform = `rotate(${anti_angle-90}deg)`;
      characterAngle( anti_angle);
    }
  };
  ...
  return (
    ...
    <div className="relative h-12">
      ...
      <div
        id={comp_id}
        className={`flex rounded absolute pointer-events-none top-0 left-0 z-10 text-white w-52 px-1 py-1 my-1 text-sm cursor-pointer text-center`}
