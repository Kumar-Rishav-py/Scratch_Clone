      ...
      <svg ref={svgRef} className="h-16">
        <g>
          <path
            ...
            fill="#9966FF"
            fillOpacity="1"
            d={inputRef.current ? calculatePath(inputRef.current.offsetWidth) : calculatePath(60)}
          ></path>
        </g>
      </svg>
      <div
        id={comp_id}
        className="flex absolute top-0 left-0 z-10 flex-row flex-wrap text-center text-white px-2 py-1 my-2 text-sm cursor-pointer"
